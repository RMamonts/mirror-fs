...
use super::helpers::{
    alloc_slice, assert_wcc_present, create_dir, cred, default_new_attr, dir_op, expect_err,
    expect_ok, file_path, sized_attr, slice_from_bytes, slice_to_vec, write_file, TestContext,
};
...
async fn set_attr_updates_size_and_honors_guard() {
    ...
    let current = expect_ok(
        get_attr::GetAttr::get_attr(&ctx.fs, cred(), get_attr::Args { file: handle.clone() }).await,
